...
```{r}
# Step 2: Add color
ggplot(shots_sum, aes(x, y)) + geom_point(aes(color = avg_points))
```

## Step 3: Size the bubbles
```{r}
# Step 3: Size bubbles
ggplot(shots_sum, aes(x, y)) + geom_point(aes(color = avg_points, size = num_shots))
```
...
# graph again
ggplot(shots_sub, aes(x, y)) + geom_point(aes(color = avg_points, size = num_shots))
```

## Step 5: Change limits to focus within the 3 point boundary
```{r}
ggplot(shots_sub, aes(x, y)) + geom_point(aes(color = avg_points, size = num_shots)) + ylim(0, 35)
```

## Step 6: X and Y distance should be in same distance
```{r}
# Step 6: x and y distance should be in same distance
ggplot(shots_sub, aes(x, y)) + geom_point(aes(color = avg_points, size = num_shots)) + ylim(0, 35) + coord_equal()
```
